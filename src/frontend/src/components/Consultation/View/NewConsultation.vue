    ...
    <div class="card mt-1">
      ...
      <div>
        <div class="float-right">
          <button class="btn btn-primary" @click="saveConsultation">
        ...
        <br />
        <ProblemTab
          :selected-problems="isEditMode(consultation) ? problems : null"
          :edit="isEditMode(consultation)"
        ></ProblemTab>
      </div>
      <hr style="width:100%" />
    </div>
...
export default {
  ...
  mounted() {
    bus.$on("updateComment", event => {
      this.consultation.comment = event.target.value;
    });
    bus.$on("updateConsult", this.updateConsult);
